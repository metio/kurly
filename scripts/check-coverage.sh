...
expected="$(jq '([.features[].kinds | length] | add) + (.expose | length) + (.security | length) + (.kinds | length) + ([.network[] | select(.standalone != true)] | length) + ([.mesh[] | select(.standalone != true)] | length) + ([.backup[] | select(.standalone != true)] | length)' .build/catalog.json)"
