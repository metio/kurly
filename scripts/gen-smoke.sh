...
# readable across working sessions.
{
  ...
  printf 'Workloads still awaiting a scenario (operator/custom-resource backed, no\n'
  printf 'bootable image of their own):\n\n'
  jq -r '.workloads[] | select(.maturity.tier != "e2e") | "- \(.id)"' "$catalog" | sort
