...
# readable across working sessions.
{
  ...
  # A workload that reached `delivered` cleared `e2e` on the way, so both tiers
  # count as covered here.
  cov=$(jq '[.workloads[] | select(.maturity.tier == "e2e" or .maturity.tier == "delivered")] | length' "$catalog")
  printf '**%s / %s workloads have an e2e scenario.**\n' "$cov" "$total"
  remaining="$(jq -r '.workloads[] | select(.maturity.tier != "e2e" and .maturity.tier != "delivered") | "- \(.id)"' "$catalog" | LC_ALL=C sort)"
  if [ -n "$remaining" ]; then
