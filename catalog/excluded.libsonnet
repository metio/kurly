...
// more.
{
  ...
  lorax: { reason: 'undeployable', note: 'publishes no release tag — the registry carries `latest`, `main` and commit shas including one tagged 9801440-dirty — so there is no version to pin a workload to — https://github.com/predibase/lorax' },
  'production-stack': { reason: 'undeployable', note: 'a reference Helm stack for running vLLM at scale — a router, a cache and a set of serving engines installed together with its own chart — rather than a workload to compose; the engine it packages is vLLM itself — https://github.com/vllm-project/production-stack' },
  hiclaw: { reason: 'undeployable', note: 'renamed AgentTeams; its Helm chart installs a stack — a Higress AI gateway, a Tuwunel Matrix server, MinIO and its own controller — rather than a workload, and the manager alone runs no agents — https://github.com/higress-group/hiclaw' },
  kubero: { reason: 'undeployable', note: 'the UI and an operator are one system installed by its own `kubero install` CLI, and every application it manages is a custom resource stored in etcd rather than a database — the UI alone manages nothing — https://github.com/kubero-dev/kubero' },
