...
// memex — a memEx server (a personal knowledge base of notes, contexts and
// pipelines, inspired by zettelkasten and org-mode). A plain composable
// kurly.http workload on the official image: a Phoenix release on :4000 with all
// of its state in an external PostgreSQL, so it claims no volume. Import it,
...
//   kurly.list(memex(host='memex.example.com'))
//
// Serves the web app on :4000 — compose an exposure onto it.
//
// DATABASE: the release migrates on start (a migrator sits in the supervision
// tree), so a first boot against a fresh database is slower than the ones after
// it — that is what the startup budget is for. The connection string is
// DATABASE_URL and it carries the password, so it lives in the Secret rather than
// in env here; the defaults pair with a CNPG cluster named `memex-db`.
//
...
// SMTP_HOST, SMTP_USERNAME and SMTP_PASSWORD. memEx invites users by email and
...
function(
  name='memex',
  image=defaultImage,
  // The public domain the browser reaches this instance at. memEx builds its
  ...
  smtpPort='587',
  smtpSsl=false,
  // The sender address and display name. memEx derives the address from HOST when
  // it is not given, which is a mailbox that usually does not exist.
  emailFrom=null,
  emailName='memEx',
  // Who may sign up: 'invite' (an existing user hands out an invite link) or
  // 'public'. Invite-only by default, matching the application's own default.
  registration='invite',
