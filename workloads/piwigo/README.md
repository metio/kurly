...
kind: OCIRepository
metadata: { name: kurly, namespace: piwigo }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-piwigo, namespace: piwigo }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/piwigo, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-piwigo, namespace: piwigo }
spec: { sourceRef: { kind: OCIRepository, name: kurly-piwigo } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: piwigo, namespace: piwigo }
spec:
  serviceAccountName: piwigo-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/piwigo/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-piwigo, importPath: github.com/metio/kurly/workloads/piwigo }
```
...
metadata: { name: piwigo, namespace: piwigo }
spec:
  serviceAccountName: piwigo-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: piwigo }
```
