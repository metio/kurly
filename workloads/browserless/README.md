...
kind: OCIRepository
metadata: { name: kurly, namespace: browserless }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-browserless, namespace: browserless }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/browserless, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-browserless, namespace: browserless }
spec: { sourceRef: { kind: OCIRepository, name: kurly-browserless } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: browserless, namespace: browserless }
spec:
  serviceAccountName: browserless-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/browserless/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-browserless, importPath: github.com/metio/kurly/workloads/browserless }
```
...
metadata: { name: browserless, namespace: browserless }
spec:
  serviceAccountName: browserless-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: browserless }
```
