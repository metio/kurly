...
kind: OCIRepository
metadata: { name: kurly, namespace: calibre-web }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-calibre-web, namespace: calibre-web }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/calibre-web, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-calibre-web, namespace: calibre-web }
spec: { sourceRef: { kind: OCIRepository, name: kurly-calibre-web } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: calibre-web, namespace: calibre-web }
spec:
  serviceAccountName: calibre-web-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/calibre-web/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-calibre-web, importPath: github.com/metio/kurly/workloads/calibre-web }
```
...
metadata: { name: calibre-web, namespace: calibre-web }
spec:
  serviceAccountName: calibre-web-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: calibre-web }
```
