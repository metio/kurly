...
kind: OCIRepository
metadata: { name: kurly, namespace: docuseal }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-docuseal, namespace: docuseal }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/docuseal, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-docuseal, namespace: docuseal }
spec: { sourceRef: { kind: OCIRepository, name: kurly-docuseal } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: docuseal, namespace: docuseal }
spec:
  serviceAccountName: docuseal-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/docuseal/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-docuseal, importPath: github.com/metio/kurly/workloads/docuseal }
```
...
metadata: { name: docuseal, namespace: docuseal }
spec:
  serviceAccountName: docuseal-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: docuseal }
```
