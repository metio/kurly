// SPDX-FileCopyrightText: The kurly Authors
// SPDX-License-Identifier: 0BSD

// transmission — a a Transmission server (a fast, lightweight, self-hosted BitTorrent client with a web UI). A plain composable kurly.http workload on the LinuxServer.io
// image; its application config (SQLite) lives on a PersistentVolume. Import it and render
// with kurly.list:
//
//   local transmission = import 'github.com/metio/kurly/workloads/transmission/server.libsonnet';
//   kurly.list(transmission())
//
// Serves the web app on :9091 — compose an exposure onto it.
//
// PEER: the BitTorrent peer port :51413 (TCP and UDP) rides onto the Service beside the web
// port (the 'peer-tcp' and 'peer-udp' ports); route it (NodePort/LoadBalancer) and forward it for
// incoming connections.
//
// DOWNLOADS & MEDIA: mount your downloads (and media/watch) directories and point the app
// at them in its settings; compose the extra volumes on. The config volume holds only the
// app's own state.
//
// LINUXSERVER IMAGE: the s6-overlay init runs as root and drops to the PUID/PGID user,
// so this runs as root with a writable root filesystem — kurly keeps the rest of the
// hardening (dropped capabilities, seccomp, no privilege escalation, resource limits).
// Set puid/pgid to own the mounted files.
//
// Single writer: the config lives on a ReadWriteOnce volume, so one replica, recreated.
local kurly = import 'github.com/metio/kurly/main.libsonnet';

local version = std.rstripChars(importstr './version.txt', '\n');

function(
  name='transmission',
  image='lscr.io/linuxserver/transmission:4.1.3',
  storageSize='2Gi',
  storageClass=null,
  puid=1000,
  pgid=1000,
  timezone='UTC',
  env={},
  resources={ requests: { cpu: '100m', memory: '256Mi' }, limits: { memory: '512Mi' } },
  labels={},
  annotations={},
)
  kurly.http(name, image)
  + kurly.version(version)
  + kurly.replicas(1)
  + kurly.recreate()
  + kurly.port(9091)
  + kurly.servicePort(9091)
  + kurly.extraPort('peer-tcp', 51413)
  + kurly.extraPort('peer-udp', 51413, protocol='UDP')
  + kurly.env({ PUID: std.toString(puid), PGID: std.toString(pgid), TZ: timezone } + env)
  + kurly.rootUser()
  + kurly.writableRootFilesystem()
  + kurly.store('/config', storageSize, storageClass=storageClass)
  + kurly.readinessProbe({ tcpSocket: { port: 'http' } })
  + kurly.livenessProbe({ tcpSocket: { port: 'http' } })
  + kurly.resources(
    requests=std.get(resources, 'requests', {}),
    limits=std.get(resources, 'limits', {}),
  )
  + kurly.labels(labels)
  + kurly.annotations(annotations)
