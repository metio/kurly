...
|---|---|---|
| `name` | `photoprism` | |
| `image` | `docker.io/photoprism/photoprism:260919` | |
...
kind: OCIRepository
metadata: { name: kurly, namespace: photoprism }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-photoprism, namespace: photoprism }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/photoprism, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-photoprism, namespace: photoprism }
spec: { sourceRef: { kind: OCIRepository, name: kurly-photoprism } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: photoprism, namespace: photoprism }
spec:
  serviceAccountName: photoprism-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/photoprism/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-photoprism, importPath: github.com/metio/kurly/workloads/photoprism }
```
...
metadata: { name: photoprism, namespace: photoprism }
spec:
  serviceAccountName: photoprism-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: photoprism }
```
