...
// druid-historical — the Apache Druid Historical: it downloads the segments the
// coordinator assigns it from deep storage into a local segment cache and answers
// the broker's queries over them. This is where a Druid deployment's query
// capacity lives. Deploy it after the coordinator.
//
//   local historical = import 'github.com/metio/kurly/workloads/druid/historical.libsonnet';
//   kurly.list(historical())
//
// Serves its query API on :8083 — an internal service, not something to expose.
//
// The volume is a CACHE, not the record: the segments themselves are in deep
// storage and a historical that loses its cache downloads them again. It is still
// a ReadWriteOnce volume one process writes, so this is one replica, recreated
// (never rolled) to keep two historicals off the same cache directory.
//
...
local defaultImage = std.rstripChars(importstr './historical.image', '\n');
...
function(
  ...
  // container.
  heap='768m',
  // The historical merges results off-heap: the direct budget must cover
  ...
  directMemory='400m',
  // The segment cache, plus the working directories the entrypoint creates.
  storageSize='20Gi',
  storageClass=null,
  // How much of that volume the segment cache may fill. Druid takes this as a
  // hard budget and refuses to load a segment past it, so it belongs BELOW
  // storageSize — a cache allowed to grow to the size of its volume leaves the
  // JVM's own scratch nowhere to go.
  segmentCacheSize='15g',
  env={},
  resources={ requests: { cpu: '500m', memory: '1536Mi' }, limits: { memory: '2Gi' } },
  labels={},
  annotations={},
)
  ...
  local baseEnv = {
    ...
    druid_segmentCache_locations: '[{"path":"var/druid/segment-cache","maxSize":"%s"}]' % segmentCacheSize,
    druid_cache_sizeInBytes: '64MiB',
  ...
  + kurly.podAnnotations({ 'kurly.metio.wtf/druid-service': 'historical' })
  ...
  // it before the JVM starts — the segment cache is one of them, which is why
  // this one is a volume rather than an emptyDir.
  + kurly.store('/opt/druid/var', storageSize, storageClass=storageClass)
  // A cold JVM plus reloading whatever the segment cache already holds takes far
  // longer than a liveness probe's patience, and /status answers nothing until it
  // is done — so the wait is a startup probe rather than a stretched liveness
  // delay.
