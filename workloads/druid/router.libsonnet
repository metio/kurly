...
// druid-router — the Apache Druid Router: one front door for a deployment. It
// serves the web console, routes queries to the brokers, and proxies the
// Coordinator and Overlord APIs so an operator needs one address rather than
// three. Deploy it last.
//
//   local router = import 'github.com/metio/kurly/workloads/druid/router.libsonnet';
//   kurly.list(router())
//
// Serves the console and the proxied APIs on :8888 — this is the stage an
// exposure goes onto. DRUID SHIPS NO AUTHENTICATION until the basic-security
// extension is configured, and the console it serves can drop datasources, submit
// ingestion tasks and read every row: an exposure on its own publishes an
// administrative interface to whoever reaches it. Keep it inside the cluster, or
// authenticate in front of it.
//
// It holds no state, so scale it with kurly.replicas.
//
...
local defaultImage = std.rstripChars(importstr './router.image', '\n');
...
function(
  ...
  replicas=1,
  env={},
  // A gigabyte is not enough, which is not obvious for a service that proxies:
  // the JVM's own native footprint — thread stacks, code cache, metaspace, and GC
  // threads sized from the node's CPU count — sits outside the heap budget, and
  // when the cgroup runs out of room for it the router dies with
  // "unable to create native thread" while its heap is barely touched.
  ...
  local baseEnv = {
    ...
    // The image's router configuration asks for a hundred Jetty threads on top of
    // its own hundred, and a thread stack is native memory the heap budget does
    // not cover — the JVM dies with "unable to create native thread" long before
    // anything runs out of heap.
    druid_router_http_numMaxThreads: '20',
  ...
  + kurly.podAnnotations({ 'kurly.metio.wtf/druid-service': 'router' })
