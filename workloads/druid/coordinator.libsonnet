...
// druid-coordinator — the Apache Druid Coordinator running as Overlord too
// (`druid.coordinator.asOverlord.enabled` is on in the image's own cluster
// configuration): it decides which segments each Historical loads, balances them,
// and — as Overlord — accepts ingestion tasks and hands them to the
// MiddleManagers. It is the FIRST stage of a Druid deployment: it creates the
// metadata tables the other services read, so bring it up and let it go healthy
// before the rest.
//
//   local coordinator = import 'github.com/metio/kurly/workloads/druid/coordinator.libsonnet';
//   kurly.list(coordinator())
//
// Serves the Coordinator/Overlord API on :8081. The web console lives on the
// router stage, so an exposure normally goes there rather than here.
//
...
local defaultImage = std.rstripChars(importstr './coordinator.image', '\n');
...
function(
  ...
  + kurly.podAnnotations({ 'kurly.metio.wtf/druid-service': 'coordinator' })
  ...
  // A cold JVM plus the metadata-table bootstrap takes far longer than a liveness
  // probe's patience, and /status answers nothing until it is done — so the wait
  // is a startup probe rather than a stretched liveness delay.
