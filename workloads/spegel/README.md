...
kind: OCIRepository
metadata: { name: kurly, namespace: spegel }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-spegel, namespace: spegel }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/spegel, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-spegel, namespace: spegel }
spec: { sourceRef: { kind: OCIRepository, name: kurly-spegel } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: spegel, namespace: spegel }
spec:
  serviceAccountName: spegel-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local mirror = import 'github.com/metio/kurly/workloads/spegel/mirror.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(mirror())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-spegel, importPath: github.com/metio/kurly/workloads/spegel }
```
...
metadata: { name: spegel, namespace: spegel }
spec:
  serviceAccountName: spegel-deployer
  ...
  stages:
    - name: mirror
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: DaemonSet, name: spegel }
```
