...
kind: OCIRepository
metadata: { name: kurly, namespace: grav }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-grav, namespace: grav }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/grav, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-grav, namespace: grav }
spec: { sourceRef: { kind: OCIRepository, name: kurly-grav } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: grav, namespace: grav }
spec:
  serviceAccountName: grav-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/grav/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-grav, importPath: github.com/metio/kurly/workloads/grav }
```
...
metadata: { name: grav, namespace: grav }
spec:
  serviceAccountName: grav-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: grav }
```
