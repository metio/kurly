...
kind: OCIRepository
metadata: { name: kurly, namespace: gotenberg }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-gotenberg, namespace: gotenberg }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/gotenberg, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-gotenberg, namespace: gotenberg }
spec: { sourceRef: { kind: OCIRepository, name: kurly-gotenberg } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: gotenberg, namespace: gotenberg }
spec:
  serviceAccountName: gotenberg-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/gotenberg/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-gotenberg, importPath: github.com/metio/kurly/workloads/gotenberg }
```
...
metadata: { name: gotenberg, namespace: gotenberg }
spec:
  serviceAccountName: gotenberg-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: gotenberg }
```
