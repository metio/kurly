...
kind: OCIRepository
metadata: { name: kurly, namespace: nextcloud }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-nextcloud, namespace: nextcloud }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/nextcloud, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-nextcloud, namespace: nextcloud }
spec: { sourceRef: { kind: OCIRepository, name: kurly-nextcloud } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: nextcloud, namespace: nextcloud }
spec:
  serviceAccountName: nextcloud-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/nextcloud/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-nextcloud, importPath: github.com/metio/kurly/workloads/nextcloud }
```
...
metadata: { name: nextcloud, namespace: nextcloud }
spec:
  serviceAccountName: nextcloud-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: nextcloud }
```
