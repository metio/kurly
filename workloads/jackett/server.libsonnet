...
// jackett — a Jackett server (a proxy server that translates queries into tracker-site HTTP requests, exposing many indexers through one API). A plain composable kurly.http workload on the
...
//   kurly.list(jackett())
//
// Serves the web app and API on :9117 — compose an exposure onto it.
//
// MEDIA & DOWNLOADS: Jackett works against your media library and download-client
// directories. Mount them and point Jackett at them in its settings; compose the extra
...
function(
  name='jackett',
  image='lscr.io/linuxserver/jackett:0.24.2246',
  storageSize='2Gi',
  storageClass=null,
  puid=1000,
  pgid=1000,
