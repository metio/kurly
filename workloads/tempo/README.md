...
kind: OCIRepository
metadata: { name: kurly-tempo, namespace: tracing }
spec:
  interval: 12h
  url: oci://ghcr.io/metio/kurly/workloads/tempo
  ref: { tag: latest }
---
...
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: tempo, namespace: tracing }
spec:
  ...
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local tempo = import 'github.com/metio/kurly/workloads/tempo/server.libsonnet';
      function(storageSecret='tempo-storage')
        kurly.list(tempo(storageSecret=storageSecret))
  libraries:
