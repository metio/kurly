...
kind: OCIRepository
metadata: { name: kurly, namespace: filebrowser }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-filebrowser, namespace: filebrowser }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/filebrowser, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-filebrowser, namespace: filebrowser }
spec: { sourceRef: { kind: OCIRepository, name: kurly-filebrowser } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: filebrowser, namespace: filebrowser }
spec:
  serviceAccountName: filebrowser-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/filebrowser/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-filebrowser, importPath: github.com/metio/kurly/workloads/filebrowser }
```
...
metadata: { name: filebrowser, namespace: filebrowser }
spec:
  serviceAccountName: filebrowser-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: filebrowser }
```
