...
kind: OCIRepository
metadata: { name: kurly, namespace: jellyfin }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-jellyfin, namespace: jellyfin }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/jellyfin, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-jellyfin, namespace: jellyfin }
spec: { sourceRef: { kind: OCIRepository, name: kurly-jellyfin } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: jellyfin, namespace: jellyfin }
spec:
  serviceAccountName: jellyfin-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/jellyfin/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-jellyfin, importPath: github.com/metio/kurly/workloads/jellyfin }
```
...
metadata: { name: jellyfin, namespace: jellyfin }
spec:
  serviceAccountName: jellyfin-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: jellyfin }
```
