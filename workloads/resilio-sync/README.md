...
kind: OCIRepository
metadata: { name: kurly, namespace: resilio-sync }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-resilio-sync, namespace: resilio-sync }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/resilio-sync, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-resilio-sync, namespace: resilio-sync }
spec: { sourceRef: { kind: OCIRepository, name: kurly-resilio-sync } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: resilio-sync, namespace: resilio-sync }
spec:
  serviceAccountName: resilio-sync-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/resilio-sync/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-resilio-sync, importPath: github.com/metio/kurly/workloads/resilio-sync }
```
...
metadata: { name: resilio-sync, namespace: resilio-sync }
spec:
  serviceAccountName: resilio-sync-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: resilio-sync }
```
