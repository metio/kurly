...
kind: OCIRepository
metadata: { name: kurly, namespace: registry }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly, ref: { tag: 2026.7.29 } }
---
apiVersion: source.toolkit.fluxcd.io/v1
kind: OCIRepository
metadata: { name: kurly-registry, namespace: registry }
spec: { interval: 12h, url: oci://ghcr.io/metio/kurly/workloads/registry, ref: { tag: 2026.7.29 } }
---
...
kind: JsonnetLibrary
metadata: { name: kurly-registry, namespace: registry }
spec: { sourceRef: { kind: OCIRepository, name: kurly-registry } }
---
apiVersion: jaas.metio.wtf/v1
kind: JsonnetSnippet
metadata: { name: registry, namespace: registry }
spec:
  serviceAccountName: registry-renderer
  files:
    main.jsonnet: |
      local kurly = import 'github.com/metio/kurly/main.libsonnet';
      local server = import 'github.com/metio/kurly/workloads/registry/server.libsonnet';
      // Compose your exposure and any + features here, then render.
      kurly.list(server())
  libraries:
    - { kind: JsonnetLibrary, name: kurly, importPath: github.com/metio/kurly }
    - { kind: JsonnetLibrary, name: kurly-registry, importPath: github.com/metio/kurly/workloads/registry }
```
...
metadata: { name: registry, namespace: registry }
spec:
  serviceAccountName: registry-deployer
  ...
  stages:
    - name: server
      ...
      readyChecks:
        checks:
          - { apiVersion: apps/v1, kind: Deployment, name: registry }
```
